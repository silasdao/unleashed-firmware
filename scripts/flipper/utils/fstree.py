...
class FsNode:
    ...
    def addDirectory(self, path):
        ...
        if parent_node := self.traverse(fragments):
...
def compare_fs_trees(left: FsNode, right: FsNode):
    ...
    left_dict = {node.getPath(): node for node in walk_nodes(left)}
    right_dict = {node.getPath(): node for node in walk_nodes(right)}
